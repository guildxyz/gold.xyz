...
import { CONNECTION, EDITION, METADATA_PROGRAM_ID, PREFIX, PROGRAM_ID } from "../consts"
...
export async function getChildAccounts(auctionId: Uint8Array, auctionOwnerPubkey: PublicKey, nextEdition: number) {
...
export async function getChildMetadata(auctionOwnerPubkey: PublicKey, auctionId: Uint8Array, nextEdition: number) {
  ...
  const [childMetadataPubkey, _f] = await PublicKey.findProgramAddress(
    [PREFIX, Buffer.from(METADATA_PROGRAM_ID.toBytes()), Buffer.from(childMintPubkey.toBytes())],
    METADATA_PROGRAM_ID
  )
